...
namespace __gnu_profile
{
  ...
  inline void
  __trace_vector_statistics_iterate(const void* __obj, std::size_t __num = 1)
  {
    if (!__profcxx_init())
      return;

    _GLIBCXX_PROFILE_DATA(_S_vector_statistics)->__opr_iterate(__obj, __num);
  }
  ...
  inline void
  __trace_vector_statistics_find(const void* __obj, std::size_t __size)
  {
    if (!__profcxx_init())
      return;

    _GLIBCXX_PROFILE_DATA(_S_vector_statistics)->__opr_find(__obj, __size);
  }
